...
const StepBattlePage: React.FC<StepBattlePageProps> = ({ onGoBack, questions, onBattleEnd }) => {
  ...
  const getButtonStyle = (index: number) => {
    if (!showFeedback) return 'bg-card border-2 border-border text-foreground hover:bg-accent hover:text-accent-foreground';
    if (index === currentStep.correctAnswer) return 'bg-battle-success/20 border-2 border-battle-success text-battle-success';
    if (index === selectedAnswer && !isCorrect) return 'bg-battle-danger/20 border-2 border-battle-danger text-battle-danger';
    return 'bg-muted/50 text-muted-foreground border-2 border-muted';
  };
              ...
              {showFeedback && (
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground">
                    <ScienceText text={currentStep.explanation} />
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
