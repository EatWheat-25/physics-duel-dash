  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div className="flex justify-between text-sm mb-3 px-4">
        <span className="text-red-400 font-bold tracking-wider">OPPONENT</span>
        <span className="text-emerald-400 font-bold tracking-wider">YOU</span>
      </div>

      <div className="relative w-full h-8 rounded-full overflow-hidden border-2 border-white/10 bg-black/60 backdrop-blur-sm shadow-[inset_0_2px_8px_rgba(0,0,0,0.6)]">
        ...
        {Array.from({ length: maxSteps * 2 - 1 }, (_, i) => {
          ...
          return (
            <div
              key={i}
              className={`absolute top-1 w-0.5 h-6 transition-opacity duration-300 ${isNearCenter ? 'bg-white/40' : 'bg-white/20'
                }`}
      ...
      <div className="flex justify-between text-xs mt-3 px-2">
        <motion.div
          className={`px-3 py-1.5 rounded-lg font-bold transition-all duration-300 ${position <= -maxSteps
              ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.6)]'
              : 'text-muted-foreground bg-black/40 border border-white/10'
            }`}
          animate={{
