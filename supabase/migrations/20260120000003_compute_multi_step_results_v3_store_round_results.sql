-- 20260120000003_compute_multi_step_results_v3_store_round_results.sql
-- Persist multi-step round results to match_rounds and enforce no points on draw.
...
begin
  ...
  for i in 0..(v_step_count - 1) loop
    ...
    -- Award main-step marks only if main correct
    ...
    -- Sub-step bonus points: 0.2 per awarded correct sub-step
    ...
    v_p1_marks_points := v_p1_marks_points + v_p1_step_awarded;
    v_p2_marks_points := v_p2_marks_points + v_p2_step_awarded;
    ...
    -- UI totals (main-step correct count)
    if v_p1_part_correct then
      v_p1_parts_correct := v_p1_parts_correct + 1;
    end if;
    if v_p2_part_correct then
      v_p2_parts_correct := v_p2_parts_correct + 1;
    end if;

    -- Collect sub-answers for payload
    v_p1_sub_answers := v_p1_sub_answers || jsonb_build_array(v_p1_sub_answer);
    v_p2_sub_answers := v_p2_sub_answers || jsonb_build_array(v_p2_sub_answer);
    ...
    v_step_results := v_step_results || jsonb_build_array(
      jsonb_build_object(
        ...
        'p1SubAnswerIndex', v_p1_sub_answer,
        'p2SubAnswerIndex', v_p2_sub_answer,
  ...
  -- Round wins tracking (draw => no points)
  ...
  -- Persist per-round results for durable resync
