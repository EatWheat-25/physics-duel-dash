-- 20260117000000_update_rank_rules_v2.sql
-- Update ranked points rules (accuracy rounding to even, new deltas, cap 2000).

begin;

comment on column public.players.rank_points is
  'Ranked points for ladder progression (0..2000). Separate from matchmaking MMR.';
...
begin
  ...
  -- Round down to nearest even percent (odd values drop to prior even)
  a := a - (a % 2);
  ...
  if p_outcome = 'draw' then
    ...
    if a < 50 then return -20;
    elsif a < 60 then return -15 + floor((a - 50) / 2.0);
    elsif a < 70 then return -10 + floor((a - 60) / 2.0);
    elsif a < 80 then return -5  + floor((a - 70) / 2.0);
    elsif a < 90 then return 0 + floor((a - 80) / 2.0);
    else return 5 + floor((a - 90) / 2.0);
    end if;
