-- 20260302000002_bot_match_bot_only.sql
-- Add bot match metadata + bot-only creation path.

begin;

alter table public.matches
  add column if not exists bot_behavior text,
  add column if not exists bot_min_accuracy_pct int;

drop function if exists public.matchmake_simple_v1(uuid, text, text, boolean);
...
create or replace function public.matchmake_simple_v1(
  ...
  p_bot_only boolean default false
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  if coalesce(p_bot_only, false) then
    v_player1 := p_player_id;
    v_player2 := v_bot_id;

    -- Ensure user isn't left in queue if they were waiting.
    delete from public.matchmaking_queue
  ...
  if not found then
    -- Bot fallback: only after 60 seconds in queue.
    v_wait_seconds := floor(extract(epoch from (now() - v_user_queue.created_at)));
    if v_wait_seconds >= 60 then
      v_player1 := p_player_id;
      v_player2 := v_bot_id;
...
grant execute on function public.matchmake_simple_v1(uuid, text, text, boolean, boolean) to service_role;
