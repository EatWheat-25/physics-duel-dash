-- 20260107000002_gate_substeps_on_main_correct_v1.sql
-- Gate sub-steps on MAIN step correctness for async segment progression.
--
-- Rules:
-- - If a player answers the main segment incorrectly (or times out), they SKIP all sub-steps
--   and immediately advance to the next main step.
-- - If main segment is correct and sub-steps exist, the player enters sub-steps (segment='sub').
-- - While in sub-steps, the player continues through all sub-steps regardless of correctness.
--
-- Notes:
-- - Keeps the default sub-step timer at 15s (honors per-(sub)step overrides via timeLimitSeconds/time_limit_seconds).
-- - This migration only changes progression; scoring is handled separately in compute_multi_step_results_v3.

begin;

-- ---------------------------------------------------------------------------
-- submit_segment_v1: dynamic step count (supports subSteps[] + legacy subStep)
-- Change: enter sub-steps ONLY when main segment is correct (and not timed out)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_segment = 'main'
     and v_sub_count > 0
     and not v_timed_out
     and coalesce(p_is_correct, false) then
    -- Enter first sub-step (ONLY when main is correct)
    ...
    -- Advance to next sub-step within the same step (independent of correctness)
    v_next_step_index := p_step_index;
    v_next_segment := 'sub';
...
-- Change: NEVER advance main -> sub on timeout (sub-steps are only entered after a correct main answer)
-- ---------------------------------------------------------------------------
...
begin
  ...
  loop
    ...
    -- NOTE: Sub-steps are only entered after a CORRECT main answer (submit_segment_v1).
    -- Since auto-advance only runs for overdue segments, this main segment is necessarily a timeout (incorrect),
    -- so we NEVER advance main -> sub here.
    if v_progress.current_segment = 'sub' and v_sub_count > 0 and (v_progress.current_sub_step_index + 1) < v_sub_count then
