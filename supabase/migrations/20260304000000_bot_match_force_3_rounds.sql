-- Force bot matches (bot_behavior = 'idle') to always play exactly 3 rounds.
-- Match-over for bot matches is determined by current_round_number >= 3,
-- not by round wins reaching target_rounds_to_win.
-- The finish_match trigger then recalculates the winner based on accuracy > 70%.

begin;

-- ============================================================================
-- compute_multi_step_results_v3: add bot-match branch
-- ============================================================================
...
begin
  ...
  -- Bot match: always play exactly 3 rounds, then finish.
  -- finish_match trigger will recalculate winner based on accuracy threshold.
  v_is_bot_match := (v_match.bot_behavior = 'idle');
  v_current_round := coalesce(v_match.current_round_number, 1);

  if v_is_bot_match then
    if v_current_round >= 3 then
      v_finished := true;
      -- Default winner to human (non-bot player); finish_match overrides via accuracy
      if v_match.player1_id = '8f7f6c2a-1f4d-4f0b-9b7d-1a2b3c4d5e6f'::uuid then
    ...
    -- Normal match: first to target_rounds_to_win and strictly ahead
...
-- ============================================================================
-- submit_round_answer_v2: add bot-match branch
-- ============================================================================
...
begin
  ...
  -- Bot match: always play exactly 3 rounds
  v_is_bot_match := (v_match.bot_behavior = 'idle');
  v_current_round := coalesce(v_match.current_round_number, 1);
  ...
  insert into public.match_step_answers_v2 (
    ...
    (p_match_id, v_round_index, v_round.question_id, v_match.player1_id,
     0, 'main', 0, v_p1_answer, v_p1_correct, 0),
    (p_match_id, v_round_index, v_round.question_id, v_match.player2_id,
     0, 'main', 0, v_p2_answer, v_p2_correct, 0)
...
-- ============================================================================
-- force_timeout_stage3: add bot-match branch
-- ============================================================================
