-- Allow solo-challenge rows (no match, no opponent) in history table.
alter table public.player_rank_points_history
...
begin
  ...
  if v_accuracy_pct > 70 then
    v_outcome := 'win';
  else
    v_outcome := 'loss';
  end if;

  v_delta := public.points_from_outcome_accuracy_v1(v_outcome, v_accuracy_pct::numeric);
